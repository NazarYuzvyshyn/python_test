...
def wait_element_visible(name, by_what: By, locator, timeout):
    try:
        wait = WebDriverWait(WebDriverFactory.driver(), timeout)
        wait.until(visibility_of_element_located((by_what, locator)))
        return True
...
def wait_list_elements(name, by_what: By, locator, timeout):
    try:
        wait = WebDriverWait(WebDriverFactory.driver(), timeout)
        wait.until(presence_of_all_elements_located((by_what, locator)))
        return True
    except TimeoutException:
